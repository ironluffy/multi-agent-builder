...
The text the user typed after `/spec-mix.specify` in the triggering message **is** the feature description. Assume you always have it available in this conversation even if `{ARGS}` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
7. Report completion with branch name, spec file path, checklist results, and readiness for the next phase (`/spec-mix.clarify` or `/spec-mix.plan`).
